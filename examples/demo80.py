...
class ManageItems(Parser):
    ...
    database_uri = Argument(
        "--db", "-D", help="Database URI", default="http://localhost:8080"
    )
...
class ForwardCommand(Parser):
    ...
    kind = Argument(
        "kind", choices=["phone", "computer", "camera", "tablet"], help="Kind of item"
    )
    action = Argument(
        "action", choices=["add", "remove", "show"], help="Action to perform"
    )
