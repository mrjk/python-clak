...
import tests.features.demo_features.demo104_nested_deep as example
...
@pytest.mark.parametrize("cli_args, expected_output, expected_exit", TEST_PARAMETERS)
def test_demo104_basic_minimal_script_regression(
    capsys, data_regression, cli_args, demo104_file, expected_output, expected_exit
):
    """Regression test that executes the actual script file using sh library."""

    output, exit_code = run_cli_script(demo104_file, cli_args)
    output = replace_with_placeholders(output)
