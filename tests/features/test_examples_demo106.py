"""
Test examples from demo106 (mixed inheritance pattern).
"""
...
TEST_PARAMETERS = [
    ...
    # Test command2 inheriting source from command1
    (["command2", "--source", "River", "John"], "Run Command 2: World", 0),
...
def test_demo106_inherit_mixed_cli(
    demo106_app, capsys, cli_args, expected_output, expected_exit
):
    """Test various CLI combinations for the mixed inheritance example."""
...
@pytest.mark.parametrize("cli_args, expected_output, expected_exit", TEST_PARAMETERS)
def test_demo106_inherit_mixed_cli_regression(
    demo106_app, capsys, data_regression, cli_args, expected_output, expected_exit
):
...
@pytest.mark.parametrize("cli_args, expected_output, expected_exit", TEST_PARAMETERS)
def test_demo106_basic_minimal_script_regression(
    capsys, data_regression, cli_args, demo106_file, expected_output, expected_exit
):
    """Regression test that executes the actual script file using sh library."""

    output, exit_code = run_cli_script(demo106_file, cli_args)
    output = replace_with_placeholders(output)
